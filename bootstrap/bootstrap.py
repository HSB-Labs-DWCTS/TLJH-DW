        ...
        return (
            subprocess.check_output(
                [
                    "/bin/bash",
                    "-c",
                    "source /etc/os-release && echo ${{{key}}}".format(
                        key=key),
                ]
            )
            .decode()
...
def _resolve_git_version(version):
    ...
    if not found:
        raise Exception(
            "No version matching {} found {}".format(
                version, sorted(all_versions))
        )
    return ".".join(str(f) for f in found)


def installExtension():
    subprocess.run(["sudo", "tljh-config", "set", "user_environment.default_app", "jupyterlab"], shell=True, check=True)
...
def main():
    ...
    if args.show_progress_page:
        ...
        urllib.request.urlretrieve(
            progress_page_favicon_url, "/var/run/favicon.ico")
        ...
        try:
            ...
            progress_page_server = HTTPServer(
                ("", 80), ProgressPageRequestHandler)
            ...
            tljh_installer_flags.extend(
                ["--progress-page-server-pid", str(p.pid)])
        except OSError:
            pass
